...
namespace BankingSystem.Core.Repositories
{
    public class OperatorRepository : IOperatorRepository
    {
        ...
        public async Task<bool> AddOperator(CreateOperatorRequest request)
        {
            ...
            return await _dataManager.Execute(sql, request) > 0;
        }

        public async Task<bool> OperatorExists(string username)
        {
          string sql = "SELECT * FROM Operator WHERE Username = @Username";

            var data = await _dataManager.Query<Operator, object>(sql, new { Username = username });
